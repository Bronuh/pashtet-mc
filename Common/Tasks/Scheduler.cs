...
public sealed class Scheduler
{
    ...
    public CoroutineTask RunCoroutine(IEnumerator<ITiming> coroutine, ConditionDelegate runCondition = null)
    {
    ...
    private void ProcessTasks()
    {
        ...
        foreach (var task in _pendingTasks)
        {
            ...
            {
                _log.Warn($"Exception thrown while processing task {task}: {e}");
                _log.Error();
            }
        }
...
{
    public TaskStateTracker State { get; } = new();
...
/// </summary>
public sealed class CoroutineTask : IPendingTask
{
    private readonly IEnumerator<ITiming> _coroutine;

    /// <summary>
    /// Represents a task that runs a coroutine.
    /// </summary>
    /// <param name="coroutine">The coroutine to be run by the task.</param>
    public CoroutineTask(IEnumerator<ITiming> coroutine)
    {
        _coroutine = coroutine;
    }

    public TaskStateTracker State { get; } = new();
    private HashSet<Func<bool>> _validators { get; } = new();
    public bool IsAtatched { get; private set; }
    public bool IsValid => _validators.All(v => v?.Invoke() ?? false);
    ...
    public ITiming Run(SchedulerState state)
    {
        ...
        {
            var nextStep = _coroutine.Current;
            return nextStep;
        }
    ...
    public void AddValidator(Func<bool> validator) => _validators.Add(validator);
}
...
/// </summary>
public sealed class TaskStateTracker
{
