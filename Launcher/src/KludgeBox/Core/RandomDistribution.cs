...
public class RandomDistribution
{
	...
	// Cached probability constants. Another values are being interpolated between cached ones.
	...
	public RandomDistribution(RandomDistributionState state)
	{
		if(state is null)
			throw new ArgumentNullException(nameof(state));

		_procChance = state.ProcChance;
		_constants = state.Constants;
		_constant = state.Constant;
	...
	public RandomDistributionState GetState()
	{
		return new RandomDistributionState(_constants, _constant, _procChance, CurrentChance);
	}
}

public record RandomDistributionState(double[] Constants, double Constant, double ProcChance, double CurrentChance);